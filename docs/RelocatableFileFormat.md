...
This is an extension mechanism that allows to define additional link items, the items generated by MACRO-80 and Nestor80 are listed in ["Extension link items"](#extension-link-items).
...
This item forces a byte boundary, this means that after the item has been written to the relocatable file as many zero bits are added so that the total number of written bits is a multiple of 8.
