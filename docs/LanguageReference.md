...
⚠ When the location counter overflows (its value goes beyond FFFFh) it will go back to 0 and a warning will be generated. Nestor80 can't generate files larger than 64KBytes.
...
Nestor80 supports Z80 and R800 🆕 CPU instructions, being Z80 the default CPU; a `.CPU R800` instruction can be used to enable support for R800 instructions. The 8080 CPU is not supported by Nestor80. 🚫
