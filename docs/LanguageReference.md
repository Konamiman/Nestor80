...
Nestor80 allows to generate a listing with information about the generated addresses and symbols, displayed together with the source code. A listing is generated if the `--listing` argument is supplied to Nestor80.
...
* `<title>` is the last listing title that was set with a `TITLE` instruction, by default is empty.
* `<subtitle>` is the last listing title that was set with a `SUBTTL` instruction, by default is empty.
...
`<address>` is a four digit hexadecimal numbers that is possibly followed by a prefix:

* No prefix: absolute address.
...
By default the `<bytes>` area will only display four bytes, and any remaining bytes (e.g. for long `DEFB` instructions) will appear below in extra lines, without source line. The amount of bytes per line can be configured, see "Listing related Nestor80 arguments".
