...
{
    /// <summary>
    /// This class allows to generate listing files with a format thatis very similar
    /// to the listing files generated by Macro80.
    /// </summary>
    public static class ListingFileGenerator
    {
        ...
        /// <summary>
        /// Generate a Macro80-style listing file.
        /// </summary>
        /// <param name="assemblyResult">The information to use to generate the listing.</param>
        /// <param name="writer">A stream writer to write the listing data to.</param>
        /// <param name="config">The listing generation configuration object to use.</param>
        ...
        private static void ProcessLine(ProcessedSourceLine processedLine)
        {
            ...
            if(processedLine is ConstantDefinitionLine cdl) {
                ...
                if(lcl.Type is ListingControlInstructionType.Lall) {
                    listMacroExpansionProducingOutput = true;
                    listMacroExpansionNotProducingOutput = true;
                }
                else if(lcl.Type is ListingControlInstructionType.Sall) {
                    listMacroExpansionProducingOutput = false;
                    listMacroExpansionNotProducingOutput = false;
                }
                else if(lcl.Type is ListingControlInstructionType.Xall) {
                    listMacroExpansionProducingOutput = true;
                    listMacroExpansionNotProducingOutput = false;
                }
                else if(lcl.Type is ListingControlInstructionType.Lfcond) {
                    currentlyListingFalseConditionals = true;
                }
                else if(lcl.Type is ListingControlInstructionType.Sfcond) {
                    currentlyListingFalseConditionals = false;
                }
                else if(lcl.Type is ListingControlInstructionType.Tfcond) {
                    tfcondState = !tfcondState;
                    currentlyListingFalseConditionals = tfcondState;
                }
                else if(lcl.Type is ListingControlInstructionType.List) {
                    listingActive = true;
                }
                else if(lcl.Type is ListingControlInstructionType.XList) {
                    listingActive = false;
                }
            }
            ...
            while(bytesRemainingInRow > 0) {
                if(nextRelocatableIndex < currentOutputProducerLine.RelocatableParts.Length && currentOutputProducerLine.RelocatableParts[nextRelocatableIndex].Index == outputIndex) {
                    ...
                    if(relocatable is LinkItemsGroup) {
                        ...
                        var relocatableAddress = (RelocatableValue)relocatable;
